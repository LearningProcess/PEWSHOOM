...
Image Img = Image(mat_name.cols, mat_name.rows, false);//Image Img;// TODO revisit the need for Img
memcpy(Img.buf, mat_name.ptr(), mat_name.rows*mat_name.cols);//sLoadImage(Img, sPath);
switch (DetAttr)
    {
    case FA_ViolaJones:
    ...
    case FA_ViolaJones:
        switch (CONF_nVjMethod)
            ...
            case 1:
                ...
                break;
            default:
                Err("GetShapeFromGlobalDet: bad nVjMethod %d", CONF_nVjMethod);
            }
        break;
    case FA_Rowley:
        switch (CONF_nRowleyMethod)
            {
            case 0:
                if (DetParams.lex == INVALID || DetParams.rex == INVALID)
                    {
                    ...
                    if (!CONF_fRowleyUseEvenIfEyeMissing)
                        {
                        lprintf("(skipping image) ");
                        return false;           // NOTE: return
                        }
                    else if (DetAv.nrows() > 0) // valid DetAv?
                        AlignStartShapeToDet(StartShape,
                                             DetParams, DetAv, CONF_RowleyScale);
                    else
                        {
                        lprintf("(skipping image) ");
                        return false;           // NOTE: return
                    ...
                    AlignToHandRowley(StartShape, DetParams, MeanShape);
                break;
            case 1:
                AlignStartShapeToDet(StartShape,
                                     DetParams, DetAv, CONF_RowleyScale);
                break;
            default:
                Err("GetShapeFromGlobalDet: bad nRowleyMethod %d", CONF_nRowleyMethod);
            }
        break;
    default:
        Err("GetShapeFromGlobalDet: bad DetAttr 0x%x", DetAttr);
    }
