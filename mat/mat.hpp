...
// -------------------
//
// TODO are these comments still correct?
...
//      static Mat m; m.dimClear(nRows, nCols)
// is quicker than
//      Mat m(nRows, nCols)
...
extern Vec  SolveWithLU(const Mat &A, Vec &b);
extern double xAx(const Vec &x, const Mat &A);
extern bool fPosDef(const Mat &A);  // true if A is positive definite
extern Mat GetEigsForSymMat(const Mat &A, Vec &EigVals, bool fSort=true, bool fTestForSymmetry=true, bool fFixSigns=true);
extern bool fIsMatrixSymmetric(const Mat &A, double Min=0.0);
extern Vec SolveWithCholesky(Mat &A, const Vec &b, bool *pfPosDef);
...
void copy(const Mat &other)  // used for copy constructor and assignment
    {
...
Mat(): m(NULL) { setGslErrHandler(); } // default constructor

Mat(size_t nRows, size_t nCols, bool fClear=true)
    {
    setGslErrHandler();
    if (nRows == 0 || nCols == 0)
        m = NULL;
...
Mat(size_t nelems1, const char sIsRowVec[]=NULL, bool fClear=true)
...
Mat(const double Vals[], size_t nRows, size_t nCols);  // declare and init from an array

// declare as vec and init
Mat(const double Vals[], size_t nelems1, const char sIsRowVec[]=NULL);
...
void dim(const Mat &other);     // redimen to same size as other, new contents unspecified
...
void assign(const Mat &other);  // redim to same size as other and copy other into this
...
Mat reshapeMe(size_t nNewRows, size_t nNewCols, bool fRowMajor=false,
      size_t nStartRow=0, size_t nStartCol=0, size_t nRows=0, size_t nCols=0);

// return submatrix of this matrix
Mat submat(size_t nStartRow, size_t nStartCol, size_t nRows=0, size_t nCols=0)
...
Mat&        operator=(const double Val);    // assign a scalar to all elements of mat
...
Mat         map(P_MAT_FUNC pFunc) const; // applies pFunc to each elem
...
Mat    fill(const double &d)        { gsl_matrix_set_all(m, d); return *this; }
...
Mat    inverse() const;                     // return inverse, bad results if mat singular
Mat    invertMe() { assign(inverse()); return *this; }
